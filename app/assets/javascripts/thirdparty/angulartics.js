...
(function(angular, analytics) {
	...
	angulartics.waitForVendorApi = function(objectName, delay, containsField,
		registerFn, onTimeout) {
		if (!onTimeout) {
		...
		if (!Object.prototype.hasOwnProperty.call(window, objectName) || (
				containsField !== undefined && window[objectName][containsField] ===
				undefined)) {
			setTimeout(function() {
				angulartics.waitForVendorApi(objectName, delay, containsField,
					registerFn, true);
			}, delay);
		} else {
			angulartics.waitForVendorCount--;
		...
		.provider('$analytics', function() {
			...
			var provider = {
				...
				withBase: function(value) {
					this.settings.pageTracking.basePath = (value) ? angular.element('base')
						.attr('href').slice(0, -1) : '';
				},
			...
			var register = function(handlerName, fn) {
				...
				var handlerDelay = (handlerSettings) ? handlerSettings.bufferFlushDelay :
					null;
			...
			var installHandlerRegisterFunction = function(handlerName) {
				...
				api[handlerName] = updateHandlers(handlerName, bufferedHandler(
					handlerName));
			};
	...
	.directive('analyticsOn', ['$analytics', '$timeout', function($analytics,
		$timeout) {
		function isCommand(element) {
			return ['a:', 'button:', 'button:button', 'button:submit',
		...
		function isProperty(name) {
			return name.substr(0, 9) === 'analytics' && ['On', 'Event', 'If',
				'Properties', 'EventType'
			].indexOf(name.substr(9)) === -1;
		}
